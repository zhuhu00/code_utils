...
int main()
{
    ...
    Mat LearnCV_black = imread("../../data/LearnCV_black.png");
    Mat LearnCV_white = imread("../../data/LearnCV_white.png");
    ...
    //白背景腐蚀
    erode(LearnCV_white, erode_write1, struct1);
    erode(LearnCV_white, erode_write2, struct2);
    imshow("LearnCV_white", LearnCV_white);
    imshow("erode_write1", erode_write1);
    imshow("erode_write2", erode_write2);

    //验证腐蚀对小连通域的去除
    Mat img = imread("../../data/rice.png");
    if (img.empty())
    {
